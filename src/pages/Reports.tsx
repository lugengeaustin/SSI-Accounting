      ...
      {loading ? <Loading /> : (
        <table className="w-full border-collapse">
          ...
          <tbody>
            ...
            <tr><td className="td"></td><td className="td"></td><td className="td text-right font-bold">Totals</td><td className="td mono text-right font-bold">{num(td)}</td><td className="td mono text-right font-bold">{num(tc)}</td></tr>
          </tbody>
        </table>
      )}
    </Modal>
...
function PnlReport({ from, to }: { from: string; to: string }) {
  ...
  const Section = ({ title, rs }: { title: string; rs: typeof rows }) => (
    <Card className="mb-3.5"><div className="border-b border-line px-[18px] py-3.5"><h3 className="text-[15px] font-bold">{title}</h3></div>
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between gap-4 rounded-[10px] bg-gradient-to-r from-brand-blue to-brand-blued px-[22px] py-5 text-white">
        <div><p className="m-0 opacity-85">Net surplus / (deficit) · {from} → {to}</p><h3 className="mono m-0 text-[18px] font-bold">{money(tr - te)}</h3></div>
    ...
    <>
      ...
      <Card><div className="overflow-x-auto"><table className="w-full border-collapse">
        ...
        <tbody>
          ...
          <tr><td className="td"></td><td className="td text-right font-bold">Totals (as at {asAt})</td><td className="td mono text-right font-bold">{num(td)}</td><td className="td mono text-right font-bold">{num(tc)}</td></tr>
        </tbody>
      </table></div></Card>
...
function BalanceSheetReport({ asAt }: { asAt: string }) {
  ...
  const Section = ({ title, rs, total, drillable }: { title: string; rs: typeof rows; total: number; drillable?: boolean }) => (
    <Card className="mb-3.5"><div className="flex justify-between border-b border-line px-[18px] py-3.5"><h3 className="text-[15px] font-bold">{title}</h3><b className="mono">{num(total)}</b></div>
      <div className="overflow-x-auto"><table className="w-full border-collapse"><tbody>
  ...
  return (
    <>
      ...
      <div className={`flex items-center justify-between rounded-[10px] px-[22px] py-4 text-white ${Math.abs(tA - (tL + tE)) < 0.01 ? "bg-brand-green" : "bg-brand-red"}`}>
        <span>Assets {num(tA)} {Math.abs(tA - (tL + tE)) < 0.01 ? "=" : "≠"} Liabilities + Equity {num(tL + tE)}</span>
        <span className="font-bold">{Math.abs(tA - (tL + tE)) < 0.01 ? "✓ Balanced" : "Out of balance"}</span>
      </div>
      {drill && <DrillModal account={drill.code} name={drill.name} to={asAt} onClose={() => setDrill(null)} />}
    </>
  );
    ...
    <>
      ...
      <Card><div className="overflow-x-auto"><table className="w-full border-collapse">
        ...
        <tbody>
          ...
          <tr><td className="td text-right font-bold">Net cash movement</td><td className="td mono text-right font-bold">{num(ti)}</td><td className="td mono text-right font-bold">{num(to2)}</td><td className="td mono text-right font-bold">{num(ti - to2)}</td></tr>
        </tbody>
      </table></div></Card>
  ...
  return (
    <>
      ...
      <Card><div className="overflow-x-auto"><table className="w-full border-collapse">
        ...
        <tbody>{rows.length ? rows.map((r) => (<tr key={r.period || ""}><td className="td mono">{r.period}</td><td className="td mono text-right">{num(r.output_vat)}</td><td className="td mono text-right">{num(r.input_vat)}</td><td className="td mono text-right font-semibold">{num(r.net_vat)}</td></tr>)) : (<tr><td className="td p-8 text-center text-muted" colSpan={4}>No VAT activity yet.</td></tr>)}</tbody>
      </table></div></Card>
    </>
  );
}
...
function WhtReport() {
  ...
  return (
    <div className="grid grid-cols-2 gap-4 max-[980px]:grid-cols-1">
      <Card className="p-5"><div className="text-[12px] uppercase tracking-wide text-muted">WHT Receivable (clients withheld)</div><div className="mono mt-2 text-[26px] font-bold">{num(rec)}</div><p className="mt-2 text-[13px] text-muted">Claim as a tax credit on your TRA return.</p></Card>
      <Card className="p-5"><div className="text-[12px] uppercase tracking-wide text-muted">WHT Payable (you withheld)</div><div className="mono mt-2 text-[26px] font-bold">{num(pay)}</div><p className="mt-2 text-[13px] text-muted">Remit to TRA and issue certificates to payees.</p></Card>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <Card><div className="overflow-x-auto"><table className="w-full border-collapse">
        ...
        <tbody>{rows.length ? rows.map((r) => (<tr key={r.id || ""}><td className="td mono">{r.invoice_no}</td><td className="td">{r.client_name}</td><td className="td">{r.issue_date}</td><td className="td mono text-right">{num(r.total)}</td><td className="td mono text-right"><span className={Number(r.days_outstanding) > 30 ? "font-bold text-brand-red" : ""}>{r.days_outstanding}</span></td><td className="td text-muted">{r.status}</td></tr>)) : (<tr><td className="td p-8 text-center text-muted" colSpan={6}>No outstanding receivables.</td></tr>)}</tbody>
      </table></div></Card>
    </>
  );
}
  ...
  return (
    <>
      ...
      {projects.length ? projects.map((pn) => {
        ...
        return (
          <Card key={pn} className="mb-3.5"><div className="border-b border-line px-[18px] py-3.5"><h3 className="text-[15px] font-bold">{pn}</h3></div>
            <div className="overflow-x-auto"><table className="w-full border-collapse">
              ...
              <tbody>{rs.map((r) => { const v = Number(r.budget_amount || 0) - Number(r.actual || 0); return (<tr key={r.account_code || ""}><td className="td">{r.account_name}</td><td className="td mono text-right">{num(r.budget_amount)}</td><td className="td mono text-right">{num(r.actual)}</td><td className="td mono text-right"><span className={v < 0 ? "font-bold text-brand-red" : "text-brand-green"}>{num(v)}</span></td></tr>); })}</tbody>
            </table></div>
          </Card>
        );
  ...
  return (
    <>
      ...
      <Card><div className="overflow-x-auto"><table className="w-full border-collapse">
        ...
        <tbody>{rows.length ? rows.map((i) => (<tr key={i.id || ""}><td className="td mono">{i.imprest_no}</td><td className="td">{i.officer_name}</td><td className="td mono text-right">{num(i.amount_issued)}</td><td className="td mono text-right">{num(i.amount_spent)}</td><td className="td mono text-right">{num(i.balance)}</td><td className="td mono text-right"><span className={Number(i.age_days) > 14 ? "font-bold text-brand-red" : ""}>{i.age_days}d</span></td></tr>)) : (<tr><td className="td p-8 text-center text-muted" colSpan={6}>No outstanding imprests.</td></tr>)}</tbody>
      </table></div></Card>
    </>
  );
}
